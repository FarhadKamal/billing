	...
	function FinalSubmit($id)
	{
		...
		if (in_array($rows->company, $comp)) {
			...
			if ($this->session->userdata('username') == 2346) {
				...
				if (in_array($rows->company, array(4, 12)) and $rows->amount > 50000) {
			...
			} else if ($this->session->userdata('username') != 2571 and in_array($rows->company, array(22, 23))) {
			...
			} else if ($this->session->userdata('username') == '003' and in_array($rows->company, array(4, 12))  and $rows->dgm_accept_by == 2346) {
		...
		if (in_array($rows->company, array(6, 21, 24))) {
