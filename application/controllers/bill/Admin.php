...
Class Admin extends MY_Controller {
	...
	function __construct(){
		...
		if ($this->userauth->is_logged_in() AND $this->session->userdata('authlevel') != 1  AND $this->session->userdata('authlevel') != 666 AND $this->session->userdata('authlevel') != 9 AND $this->session->userdata('authlevel') != 7 AND $this->session->userdata('authlevel') != 8 AND $this->session->userdata('username') !=1109  AND $this->session->userdata('username') !=1091 AND $this->session->userdata('username') !=1508 AND $this->session->userdata('username') !=1775   AND $this->session->userdata('username') !=1936 AND $this->session->userdata('username') !=1087 AND $this->session->userdata('username') !=1580 AND $this->session->userdata('username') !=2346 
		AND $this->session->userdata('username') !=2405 ) {
	
		show_401();
	}

	if(strtolower($this->session->userdata('username'))=='dhkhead' or strtolower($this->session->userdata('username'))=='amar' or strtolower($this->session->userdata('username'))=='man' or strtolower($this->session->userdata('username'))=='sharma'){
	...
	function viewDocs($id){
		$data['userlevel']=$this->session->userdata('authlevel');
		$data['docs']=$this->adminmodel->get_documents($id);
	...
	function get_doc_details()
	{
		...
		$row=$this->adminmodel->get_by_id($bill_id)->row();
		...
		foreach($billdoc->result() as $rows)
					...
					$doccount=$doccount+1;
							...
							else $viewbilldoc=	$viewbilldoc.'<a href="'.base_url().'index.php/bill/admin/url/'.$rows->doc_file.'" target="about_blank" class="view">document-'.$docsl.'</a><br/>';
						$docsl=$docsl+1;
					}
		...
		foreach($billdoc->result() as $rows)
					...
					else $report=base_url().'index.php/bill/admin/url/'.$rows->doc_file;//'ftp://billmaster:stargold@192.168.1.117/Common_share/'.$rows->doc_file;
	...
	function billList($offset = 0,$message=''){
		...
		$data['employee']=$this->adminmodel->list_employeeid();
		$data['company']=$this->adminmodel->list_company();
		//Search 
		$data['action']=site_url('bill/admin/searchbill');
		$data['action2']=site_url('bill/admin/searchbillEmp');
		$data['action3']=site_url('bill/admin/searchSpecialbill');
		$data['action4']=site_url('bill/admin/searchAmount');
		...
		// load data
		$bill = $this->adminmodel->get_paged_list($this->limit, $offset)->result();
		
		// generate pagination
		$this->load->library('pagination');
		
		$config['base_url'] = site_url('bill/admin/billList/');
		$number_of_rows=$this->adminmodel->count_all();
	...
	// generate table data
		...
		$viewbilldoc="";
			...
			//$vCompany=$this->adminmodel->get_company_by_id($row->company_id)->row()->vCompany;
	...
	$data['company']=$this->adminmodel->list_company();
		...
		// load data
		$bill = $this->adminmodel->get_search_by_id($this->mkDate($this->input->post('from')),$this->mkDate($this->input->post('to')),$this->input->post('status'),$this->input->post('loc'),$this->input->post('company'));
	...
	$data['company']=$this->adminmodel->list_company();
		...
		// load data
		$bill = $this->adminmodel->get_search_by_id_by_emp($this->input->post('EmpId'));
	...
	$data['company']=$this->adminmodel->list_company();
		...
		$bill = $this->adminmodel->get_special_search_by_id($this->input->post('stype'),$this->input->post('svalue'));
	...
	$data['company']=$this->adminmodel->list_company();
		...
		$bill = $this->adminmodel->get_search_by_amount($this->input->post('amtfrom'),$this->input->post('amtto'));
	...
	function requisitionList($offset = 0,$message=''){
		...
		$data['action']=site_url('bill/admin/searchrequisition');
		
		// load data
		$requisition = $this->adminmodel->all_material_list($this->limit, $offset)->result();
		
		// generate pagination
		$this->load->library('pagination');
		$config['base_url'] = site_url('bill/admin/requisitionList/');
		$number_of_rows=$this->adminmodel->count_material();
 		$config['total_rows'] = $number_of_rows;
 		$config['per_page'] = $this->limit;
		$config['uri_segment'] = $uri_segment;
		$this->pagination->initialize($config);
	...
	$data['employee']=$this->adminmodel->list_employeeid();
		...
		// load data
		$requisition = $this->adminmodel->get_requisition_search($this->input->post('stype'),$this->input->post('svalue'))->result();
	...
	function billPaymentList($offset = 0,$message=''){
		...
		//Search 
		$data['action']=site_url('bill/admin/searchpaymentbill');
		...
		// load data
		$bill = $this->adminmodel->get_paged_payment_list($this->limit, $offset)->result();
		
		// generate pagination
		$this->load->library('pagination');
		$config['base_url'] = site_url('bill/admin/billPaymentList/');
		$number_of_rows=$this->adminmodel->count_payment_all();
 		$config['total_rows'] = $number_of_rows;
 		$config['per_page'] = $this->limit;
		$config['uri_segment'] = $uri_segment;
		$this->pagination->initialize($config);
	...
	function searchpaymentbill(){
		...
		// load data
		$bill = $this->adminmodel->count_payment_search($this->mkDate($this->input->post('from')),$this->mkDate($this->input->post('to')),$this->input->post('loc'),$this->input->post('payment_type'))->result();

		// generate pagination
		$this->load->library('pagination');
		$data['pagination'] = '';
		$data['totalrecord'] = $this->adminmodel->count_search_payment_all($this->mkDate($this->input->post('from')),$this->mkDate($this->input->post('to')),$this->input->post('loc'),$this->input->post('payment_type'));
	...
	// generate table data
		...
		if($row->bill_type=="vendor" or $row->bill_type=="distribution"){
		//$billdoc=$this->adminmodel->get_documents($row->id);
		$billdoc=$this->billmodel->get_special_documents($row->id);			
		foreach($billdoc->result() as $rows)
					{ 
		...
		foreach($billdoc->result() as $rows)
			...
			$assignSAPid=anchor('bill/admin/assignSAPid/'.$row->id,'Assign&nbsp;SAP&nbsp;ID',array('class'=>'add'));		
	...
	function cost_table($items)
	{
		...
		$ac=site_url('bill/admin/assignSAPidENTRY/'.$row->doc_id.'/'.$row->id);
	...
	function assignSAPidENTRY($id1,$id2)
	{
			...
			redirect('bill/admin/assignSAPid/'.$id1.'/2', 'location');
	...
	function advancePaymentList($offset = 0,$message=''){
		...
		//Search 
		$data['action']=site_url('bill/admin/searchAdvancePaymentList');
		...
		// load data
		$bill = $this->adminmodel->get_paged_list_advance_payment($this->limit, $offset)->result();
		
		// generate pagination
		$this->load->library('pagination');
		$config['base_url'] = site_url('bill/admin/advancePaymentList/');
		$number_of_rows=$this->adminmodel->count_all_advance_payment();
 		$config['total_rows'] = $number_of_rows;
 		$config['per_page'] = $this->limit;
		$config['uri_segment'] = $uri_segment;
		$this->pagination->initialize($config);
	...
	function searchAdvancePaymentList(){
		...
		// load data
		$bill = $this->adminmodel->get_search_by_id_advance_payment($this->mkDate($this->input->post('from')),$this->mkDate($this->input->post('to')))->result();

		// generate pagination
		$this->load->library('pagination');
		$data['pagination'] = '';
		$data['totalrecord'] = $this->adminmodel->count_search_advance_payment($this->mkDate($this->input->post('from')),$this->mkDate($this->input->post('to')));
