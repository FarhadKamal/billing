...
class Reports extends MY_Controller
{
	...
	function view_bill_Report_For_Req()
	{
		...
		foreach ($results->result() as $mydata) :
			if ($mydata->bill_status == 0)	$completed = 'Not Completed';
	...
	function mat_items_report_by_year()
	{
		$data = $this->set_empty_param();
		$data['action'] = site_url('reports/reports/view_mat_items_report_by_year');
		$data['userlevel'] = $this->session->userdata('authlevel');
	...
	function view_mat_items_report_by_year()
	{

		$year = $this->input->post('yearID');

		$data['year']	= $year;
		$data['results'] = $this->reportsmodel->get_mat_items_report_by_year($this->input->post('yearID'));
		$data['options'] = $this->input->post('reportType');
		$this->load->view('reports/report/rpt_group_mat', $data);
	}
	...
	function view_general($id)
	{
		...
		$result = $this->reportsmodel->view_general($id);
		...
		foreach ($results3->result() as $mydata3) :
			...
			if ($result->row()->flow_type == 'Ticket') {
				$table3 = $table3 .
					'<tr>
				<td align="center"><b>Ticket:&nbsp;' . $sl . '</b></td>
				<td align="center"><b><a href="http://192.168.1.226:8989/pedrollo/index.php/reports/reports/view_ticket_status/' . $mydata3->particular . '"               target="_blank">' . $mydata3->particular . '</a></b></td>
				<td align="center"><b>' . $mydata3->particular_date . '</b></td>
				<td align="center"><b>' . $mydata3->total . '</b></td>
				<td align="center"><b>' . $str_scan . '</b></td>
				</tr>';
	...
	function order_material_list($doc_id, $detail_id, $master_id, $bill_by)
	{
		...
		$req =	$this->reportsmodel->get_req_map_id($doc_id, $master_id, $bill_by)->row();

		if ($req->req_source == "Inventory")


			redirect('http://202.51.178.202/warehouse/view_requisition.php?id=' . $master_id);
	...
	function pending_cheque()
	{
		...
		foreach ($results->result() as $mydata) {
			...
			if ($mydata->cheque_date == '') {
				...
				$table = $table . '<td align="center"><font size=2><b>' . $mydata->suggested_cheque . '<br/> Cheque&nbsp;Date:&nbsp;' . $mydata->cheque_date . '<br/>
