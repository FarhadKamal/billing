...
<div class="content" >
		...
		<table border="0" style="width: 400px;">
			...
			<tr>
				<td width="30">Reporting&nbsp;To</td>
				<td><select name="reporting_to" id="<?php echo $id;?>">
				<option  value=""><?php echo '---SELECT---';?></option>
				<?php foreach ($boss_list->result() as $row) { ?>
				
					<?php if($reportTo==$row->EmpId) { ?>
						<option  value="<?php echo $row->EmpId; ?>" selected ><?php echo $row->EmpId.'##'.$row->emp_name; ?></option>
					<?php } else if($this->validation->reporting_to==$row->EmpId){ ?>
						<option  value="<?php echo $row->EmpId; ?>" selected ><?php echo $row->EmpId.'##'.$row->emp_name; ?></option>
					<?php }else{ ?>
						<option  value="<?php echo $row->EmpId; ?>"  ><?php echo $row->EmpId.'##'.$row->emp_name; ?></option>
					<?php } ?>
