...
<div class="content" >
		...
		<table border="0" style="width: 400px;">				
			...
			<tr>
				...
				<td>	
			    <select name="CostCentre" ><option value="">-&nbsp;-SELECT-&nbsp;-</option>
				<?php
					foreach($costcentre->result() as $row)
					{ 
						if ($row->iId == $this->validation->CostCentre)
						{
							echo '<option value="'.$row->iId.'" selected="selected">'.$row->cost_id."&nbsp;##&nbsp;".$row->cost_text.'</option>';
						}
						else
						{
							echo '<option value="'.$row->iId.'">'.$row->cost_id."&nbsp;##&nbsp;".$row->cost_text.'</option>';
						}
					}
				?>
				</select><?php echo $this->validation->CostCentre_error; ?></td>	
			</tr>	

			<tr>
				<td>Internal&nbsp;Order&nbsp;<span style="color:red;">*</span></td>
				<td>	
			    <select name="internal_order" ><option value="">-&nbsp;-SELECT-&nbsp;-</option>
				<?php
					foreach($internal_order->result() as $row)
					{ 		
						if ($row->Order == $this->validation->internal_order)
						{
							echo '<option value="'.$row->Order.'" selected="selected">'.$row->Order.'&nbsp;###&nbsp;'.$row->Description.'</option>';
						}
						else
						{
				...
				?>
				</select><?php echo $this->validation->internal_order_error; ?></td>	
			</tr>
			<tr>
			...
			<tr>
				<td>Remark&nbsp;</td>
				<td>
					<textarea cols="40" rows="5" name="remark"  ><?php echo $this->validation->remark; ?></textarea>
				</td>		
			</tr>	
