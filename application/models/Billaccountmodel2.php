...
Class BillaccountModel extends ci_model {
	...
	function list_vendor(){	
		...
		$this->db->where('(vendor_code like "HL%" or vendor_code like "HD%")');}
		else {
	...
	function assignSAPidENTRY($updateId, $data){
		$this->db->where('id', $updateId);
	...
	function get_paged_list($limit = 10, $offset = 0){
		...
		if($ac_boss==1)
		{
			...
			else if(strtolower($this->session->userdata('username'))=='azhead'){ 
				$this->db->where('company_id in(18)');
			}
			else if(strtolower($this->session->userdata('username'))=='accounthead'){ 
				$this->db->where('company_id in(7)');
			}
			else if(strtolower($this->session->userdata('username'))=='ctgaccount'){ 
				$this->db->where('company_id in(1,3,2,5)');
			}
			else $this->db->where('company_id in(1,2,3,4,5,6,8,10,11,12,13,15,16,17,22,23)');
	...
	*/
	...
	function adjust_bill_list($type){
