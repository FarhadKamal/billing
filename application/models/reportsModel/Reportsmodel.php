...
class ReportsModel extends ci_model
{
	...
	function get_req_map_id($doc_id, $req_id, $bill_by)
	{
		$this->db->where('req_id', $req_id);
		$this->db->where('doc_id', $doc_id);
		$this->db->where('bill_by', $bill_by);
		return $this->db->get('mis_requisiton_map');
	}
	...
	function get_mat_items_report_by_year($year)
	{
		$query = "select ml.mat_name,  

		sum(if(month(rm.request_date)=1,rd.qty_req,0)) as jan,
		sum(if(month(rm.request_date)=2,rd.qty_req,0)) as feb,
		sum(if(month(rm.request_date)=3,rd.qty_req,0)) as mar,
		sum(if(month(rm.request_date)=4,rd.qty_req,0)) as apr,
		sum(if(month(rm.request_date)=5,rd.qty_req,0)) as may,
		sum(if(month(rm.request_date)=6,rd.qty_req,0)) as jun,
		sum(if(month(rm.request_date)=7,rd.qty_req,0)) as jul,
		sum(if(month(rm.request_date)=8,rd.qty_req,0)) as aug,
		sum(if(month(rm.request_date)=9,rd.qty_req,0)) as sep,
		sum(if(month(rm.request_date)=10,rd.qty_req,0)) as octb,
		sum(if(month(rm.request_date)=11,rd.qty_req,0)) as nov,
		sum(if(month(rm.request_date)=12,rd.qty_req,0)) as decm,

		sum(rd.qty_req) as total


			from mis_requisition_details  rd 
		inner join mis_requisition_master rm on rm.id=rd.master_id
		inner join material_list ml on ml.mat_name=rd.item_name
		where year(rm.request_date)='" . $year . "' and rd.bill_status=1
		group by ml.mat_name
		";
		return $this->db->query($query);
	}
