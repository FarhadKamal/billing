...
Class ExternalModel extends ci_model {
	...
	function get_paged_list($limit = 10, $offset = 0){
		...
		if($this->input->post('con')==3)
		{
			$sapid= 	$this->input->post('sapid');
			$this->db->join('mis_document_account','mis_document_account.doc_id=mis_document.id','join');
			$this->db->where('(mis_document_account.sap_id like "%'.$sapid.'%"  or  mis_document.sap_id like "%'.$sapid.'%"   )');
		}
		$this->db->where('step_status', 6);
		$this->db->where('mis_document.company_id', 1);
		$this->db->where("(bill_date>'2016-06-01' and bill_date<'2017-07-01')");
		
		if($this->input->post('con')==2)
		{
			$from= 	$this->mkDate($this->input->post('from'));
			$to= 	$this->mkDate($this->input->post('to'));
			$this->db->where("(bill_date>='$from' and bill_date<='$to')");
		}
		$this->db->order_by('id','desc');
		
		if($this->input->post('con')!=2 and $this->input->post('con')!=3)
		return $this->db->get('mis_document', $limit, $offset);
		else return $this->db->get('mis_document');
	}
