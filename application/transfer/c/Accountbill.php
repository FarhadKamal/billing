...
class Accountbill extends MY_Controller
{
	...
	function bill_transfer($id)
	{
		...
		if ($this->validation->run() == FALSE) {
			...
			if ($rows->loc <> $this->input->post('loc')) {
				...
				else  $oldLocation = 'Dhaka';

				if ($this->input->post('loc') == 1)
					$newLocation = 'Chittagong';
				else if ($this->input->post('loc') == 3)
					$newLocation = 'Mohakhali';
				else  $newLocation = 'Dhaka';
			...
			else $data['message'] = '<div class="success" align="left">Nothing Transfer!..</div>';
		}
	...
	function bill_table($bill)
	{
		...
		foreach ($bill as $row) {
			...
			if ($row->account_head_pass == 0 and ($row->account_pass == 1  or   $row->bill_type == "vendor" or   $row->payment_type == "Adjustment")) {
				...
				$clear = "";
				$anclear = "";
			} else $pass = anchor('bill/accountbill/holdach/' . $row->id, 'Hold', array('class' => 'add', 'onclick' => "return confirm('Are you sure want to Hold?')"));
		...
		foreach($billdoc->result() as $rows)
			...
			else if ($row->company_id == 3)
				$company = "<font color='#2DBE1A'><b>" . $row->vCompany . "</b></font>";
			else if ($row->company_id == 7)
			...
			$this->table->add_row(
				...
				$row->flow_type,
				$status,
				$viewbill . "</br>" . $clear . "<br>" . $anclear . "</br>" . $viewaction . "</br>" . $pass . "</br>" . $marked . "</br>" . $billtransfer,
				$doctable,
				$resubmit
			);
			$sl = $sl + 1;
	...
	function bill_Accept_account($id)
	{
		...
		} else if ($billtag->bill_type == "general") {
	...
	function FinalSubmit($id)
	{
		...
		} else if (in_array($billtag->company_id, $azcomp)) {
		...
		} else if (in_array($billtag->company_id, $hcomp)) {

			$this->billaccountmodel->update(
				$id,
				array(
					'account_comment' => $this->input->post('comment'), 'account_pass' => 1,	'account_head_pass' => 1, 'step_status' => 3, 'hold_by' => 'audit', 'az_status' => 1,	'hill_status' => 1
				)
			);
	...
	function adjustList($type = 'Adjustment', $message = '', $printid = 0)
	{
		// offset
	...
	function adjust_table($bill)
	{
		...
		*/
			...
			$loc = "";
			if ($row->loc == 1) $loc = "Chittagong Head Office";
			else if ($row->loc == 2) $loc = "Dhaka Head Office";
			else if ($row->loc == 3) $loc = "Mohakhali Office";
			$this->table->add_row(
				$row->id,
				$row->bill_date,
