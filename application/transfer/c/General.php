...
class General extends MY_Controller
{
	...
	function ticket()
	{
		...
		$this->validation->flow_type = "Ticket";
		$data['title'] = 'Claim General Bill';
		$data['message'] = '';
		$data['openmodel'] = 'no';
		$data['action'] = site_url('bill/general/add');
	...
	function _set_fields()
	{
		...
		$fields['invoice_type'] = 'invoice type';
	...
	{
		$data['userlevel'] = $this->session->userdata('authlevel');
		$data['docs'] = $this->generalmodel->get_documents($id);
	...
	function add()
	{
		...
		if ($this->validation->run() == FALSE) {
			...
			$this->generalmodel->save($this->makeQueryinsert());

			$data['openmodel'] = 		'yes';
			$billId = 				$this->generalmodel->get_last_entry_id($this->mkDate($this->input->post('bill_date')));

			$data['action'] =  			site_url('bill/general/updatebill/' . $billId);
			$data['itemcount'] = 		$this->generalmodel->tot_document_by_bill_id($billId);
			$data['action2'] =  site_url('bill/general/addDoc/' . $billId);
			$data['action3'] =  site_url('bill/bill/submitSupervisor/' . $billId);
			$data['action4'] =  site_url('bill/general/addParticular/' . $billId);

			$items = $this->generalmodel->get_documents($billId)->result();

			$particularitems = $this->generalmodel->get_particular($billId)->result();
			$data['table2'] =			$this->particular_table($particularitems);

			$data['table'] =			$this->documten_table($items);
			$data['list_particular'] = $this->generalmodel->list_particular($billId);

			$data['message'] = 			'<div class="success" align=left>save successful..</div>';
		}

		$data['page'] = '/billView/generalEdit'; //add page name as a parameter
		$data['userlevel'] = $this->session->userdata('authlevel');
		$data['entry'] = 1;
		$data['recommend'] = 0;
		$this->load->view('index', $data);
	...
	function getListData()
	{
		...
		$data['company'] = $this->generalmodel->list_company();
		$data['reportingOfficerList'] = $this->generalmodel->list_head();
		return $data;
	}
	...
	function documten_table($items, $link = 'a')
	{
		...
		foreach ($items as $row) {
			$table = $table . '<tr>';
			$del = '';
			if ($link != 'add_doc_general')
				$del = '&nbsp;' . anchor('bill/general/deleteDoc/' . $row->id . '/' . $row->doc_id, 'delete', array('class' => 'delete', 'onclick' => "return confirm('Are you sure want to delete?')")) . "&nbsp;&nbsp;<b>Category:</b>&nbsp;" . $row->doc_category;
	...
	// generate table data
		...
		foreach ($items as $row){
			...
			$del='&nbsp;'.anchor('bill/general/deleteDoc/'.$row->id.'/'.$row->doc_id,'delete',array('class'=>'delete','onclick'=>"return confirm('Are you sure want to delete?')"));
			//$view='&nbsp;'.anchor('docs/'.$row->doc_file,'view',array('class'=>'view','target'=>"about_blank"));
			$view='<a href="ftp://billmaster:stargold@192.168.1.117/Common_share/'.$row->doc_file.'" target="about_blank" class="view">view</a>';
			$this->table->add_row(				
				$sl,
	...
	function particular_table($items)
	{
		...
		foreach ($items as $row) {
			...
			$del = '&nbsp;' . anchor('bill/general/deleteParticular/' . $row->id . '/' . $row->doc_id, 'delete', array('class' => 'delete', 'onclick' => "return confirm('Are you sure want to delete?')"));

			//$view='&nbsp;'.anchor('docs/'.$row->doc_file,'view',array('class'=>'view','target'=>"about_blank"));
			$ac = site_url('bill/general/updateParticular/' . $row->doc_id . '/' . $row->id);
			$this->table->add_row(
	...
	function add_doc_general($id, $filechk = 1)
	{
		...
		$rows = $this->generalmodel->get_by_id($id)->row();
		...
		$this->validation->invoice_type = 					$rows->invoice_type;
		...
		$data['openmodel'] = 		'yes';
		$data['billId'] = 		$id;
		$data['itemcount'] = 		$this->generalmodel->tot_document_by_bill_id($id);
		$items = $this->generalmodel->get_documents($id)->result();
		$particularitems = $this->generalmodel->get_particular($id)->result();
		$data['table'] =			$this->documten_table($items, 'add_doc_general');
		$data['table2'] =			$this->particular_table($particularitems);
		// run validation
		if ($this->validation->run() == FALSE) {
			...
			if ($rows->step_status == 1) $this->generalmodel->update($id, $this->makeQuery());
			else if ($rows->step_status == 2) $this->generalmodel->update($id, $this->makeSuperviseQuery());
			else if ($rows->step_status == 3) $this->generalmodel->update($id, $this->makeAuditQuery());
		...
		$data['list_particular'] = $this->generalmodel->list_particular($id);
		$data['title'] = 'Claim General Bill';
		$data['action'] = site_url('bill/general/updatebill/' . $id);
		$data['action2'] =  site_url('bill/general/addDoc/' . $id);
		...
		$data['billid'] =  $id;
		$data['action4'] =  site_url('bill/general/addParticular/' . $id);
		$data['page'] = '/billView/general_doc_add'; //add page name as a parameter
		$data['userlevel'] = $this->session->userdata('authlevel');
		$data['entry'] = 1;
		$data['recommend'] = 0;
	...
	function addDoc($id)
	{
		...
		if ($this->input->post('extra_doc') == "add_doc_general")
			$linkforward = "add_doc_general";


		if ($this->input->post('pid') == "") redirect('bill/general/updatebill/' . $id . '/6', 'location');


		$ftp_server = "192.168.1.117";
		$conn_id = ftp_connect($ftp_server)
			or redirect('bill/general/' . $linkforward . '/' . $id . '/3', 'location');

		$login_result = ftp_login($conn_id, "bill", "bill007");
		if ((!$conn_id) || (!$login_result))
			redirect('bill/general/' . $linkforward . '/' . $id . '/4', 'location');
		...
		if ($doubleFileChk > 0)
			redirect('bill/general/' . $linkforward . '/' . $id . '/11', 'location');
		...
		if ($piecescount != 2) {
			ftp_close($conn_id);
			redirect('bill/general/' . $linkforward . '/' . $id . '/2', 'location');
		}
		...
		if (in_array($pieces[1], $contents_on_server) and $this->input->post('doc_file') != "") {
			...
			redirect('bill/general/' . $linkforward . '/' . $id . '/1', 'location');
		} else redirect('bill/general/' . $linkforward . '/' . $id . '/2', 'location');
	}
	...
	function addParticular($id)
	{
		...
		if ($this->input->post('particular') != "" and $this->input->post('particular_date') != ""  and $this->input->post('particular_amount') != "") {
			...
			$this->generalmodel->saveParticular(
				array(
			...
			$this->generalmodel->updateSumParticular($id);
			redirect('bill/general/updatebill/' . $id . '/1', 'location');
		} else redirect('bill/general/updatebill/' . $id . '/7', 'location');
	}
	...
	function updateParticular($id1, $id2)
	{
		...
		if ($this->input->post('particular') != "" and $this->input->post('particular_date' . $id2) != ""  and $this->input->post('amount') != "") {
			$this->generalmodel->updateParticular(
				$id2,
				array(
			...
			$this->generalmodel->updateSumParticular($id1);
			redirect('bill/general/updatebill/' . $id1 . '/1', 'location');
		} else redirect('bill/general/updatebill/' . $id1 . '/7', 'location');
	}
	...
	{
		$this->generalmodel->deleteDoc($id1);


		redirect('bill/general/updatebill/' . $id2, 'location');
	}
	...
	{
		$this->generalmodel->deleteParticular($id1);
		$this->generalmodel->updateSumParticular($id2);
		redirect('bill/general/updatebill/' . $id2, 'location');
	}
}
