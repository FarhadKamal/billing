...
class Supervisebill extends MY_Controller
{
	...
	function bill_table($bill)
	{
		...
		foreach ($bill as $row) {
			...
			if (
				$row->step_status == 2 and $row->supervise_cancel == 0 and $row->authority_cancel == 0 and $row->audit_cancel == 0

				and ($row->hold_by == $this->session->userdata('username')  or (($row->hold_by == 2430 or $row->hold_by == 3)
					and ($row->company_id == 21 or $row->company_id == 24)))
				...
				$cancel = anchor('bill/supervisebill/cancel/' . $row->id, 'cancel', array(
					'class' => 'cancel',
		...
		if ($row->bill_type == "vendor" or $row->bill_type == "distribution") {
			...
			foreach ($billdoc->result() as $rows) {
				...
				if (strlen($doc_category) < 4)
					$doc_category = "";
				if ($pchk == "start") {
					$viewbilldoc =	$viewbilldoc . $doc_category . '&nbsp<a href="' . $report . '" target="about_blank" class="view">document-' . $docsl . '</a><br/>';
	...
	function FinalSubmit($id)
	{
		...
		if (in_array($compchk, array(1, 2, 3, 8, 9))) {
			...
			if (
				$this->session->userdata('username') != 2346 and $this->session->userdata('username') != "003"
		...
		if ($this->input->post('ReportingOfficer') != "") {
			...
			if ($chk_supervise_comment == 1) {
				$this->billsupervisemodel->FinalSubmit(
					$id,
					array(
			...
			//return 0;
			...
			} else if ($this->input->post('ReportingOfficer') == 2571) {
				$this->billsupervisemodel->FinalSubmit(
					$id,
					array(
						'hold_by' => 2571, 'high_authority_by' => 2571
					)
				);
				$this->billsupervisemodel->action_doc(
					array(
				...
				if ($compchk == 6 or $compchk == 21 or $compchk == 24)
			...
			if ($this->input->post('auth_deduct') > 0) {
				$this->billsupervisemodel->save_mis_document_auth_deduction(
					$id,
					array(
	...
	*/
	...
	function canceladd($id)
	{
		...
		if ($chk_supervise_comment == 1) {
			$this->billsupervisemodel->cancel(
				$id,
				array(
					'supervise_cancel' => 1,
