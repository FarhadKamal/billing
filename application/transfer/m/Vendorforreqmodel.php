<?php
class VendorforreqModel extends ci_model
{
	...
	{
		return $this->db->query('select id,master_id,item_name,qty_req,if(qty is null,0,qty) as qty from
(select a.*,(select sum(qty) from mis_requisition_order b where  b.item_id=a.id and b.master_id=' . $req . ') as qty
 from mis_requisition_details a where a.bill_status=0 and a.master_id=' . $req . ') as details ')->result();
	...
	{
		return $this->db->query('select mis_requisiton_map.* from mis_requisiton_map
			where mis_requisiton_map.doc_id=' . $id)->result();
	}
	...
	function get_documents($billId)
	{
		...
		return $this->db->query('select mis_scan.* from mis_scan
	...
	function deleteItem($id1, $id2, $id4, $id5)
	{
		...
		$process = $this->db->query('select item_id from mis_requisition_order where bill_by="' . $id5 . '"  
		  and doc_id=' . $id2 . '  and  master_id=' . $id4)->result();
		...
		$this->db->query('delete from mis_requisition_order where bill_by="' . $id5 . '"   and doc_id=' . $id2 . '   and  master_id=' . $id4);
	}
}
